...
HITMAN = str(ALIVE_NAME) if ALIVE_NAME else "Baap"
papa = borg.uid
...
async def gban(userbot):
    ...
    await hellbot.edit(f"`🔥Global Ban IS STARTED'💦.... Wait and watch THIS NUB`")
    ...
    try:
        ...
        pass
    return await hellbot.edit(
        f"[{user.first_name}](tg://user?id={user.id}) Beta majdur ko khodna aur [{HITMAN}](tg://user?id={papa}) ko chodna kabhi sikhana nhi.\n\nGloabally tapped 🔥\nAffected Chats😏 : {a} **"
    )
