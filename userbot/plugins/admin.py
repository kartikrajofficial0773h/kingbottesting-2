...
NO_ADMIN = "`Sad yarr,I am not an admin here!`"
NO_PERM = "`I don't have sufficient permissions rights to do this! Sed loif...`"
...
async def promote(promt):
    ...
    kingevent = await edit_or_reply(promt, "`Promoting...`")
...
async def demote(dmod):
    ...
    try:
        ...
        await hellevent.edit(NO_PERM)
        return
    await kingevent.edit("`Demoted retard Successfully..... Better luck next time🚶`")
    if BOTLOG:
...
async def ban(bon):
    ...
    try:
        ...
        return
    if reason:
        await kingevent.edit(f"`{str(user.id)}` is banned !!\nReason: {reason}")
    else:
        await kingevent.edit(f"{str(user.id)} is banned😏 !!")
    if BOTLOG:
